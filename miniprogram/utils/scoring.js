...
function computeDeviationStats(frames) {
...
function detectProblemSegments(frames) {
  const segments = [];
  let active = [];
  ...
  const flush = () => {
    ...
    const averageCents = active.reduce((sum, frame) => sum + (frame.centsFromTarget || 0), 0) / active.length;
    segments.push({
      startMs,
      endMs,
